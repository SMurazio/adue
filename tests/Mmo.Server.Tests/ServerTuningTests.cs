...
public sealed class ServerTuningTests
{
    ...
    // LIVING-ENEMIES P2: the monster aggro/chase/attack knobs seed to defaults, are known-but-not-combat, apply,
    // clamp, and the DERIVED de-aggro range / attack-cooldown ticks track their source values.
    [Fact]
    public void MonsterCombatKeysSeedToDefaults()
    {
        var tuning = new ServerTuning(Options());
        Assert.Equal(6, tuning.AggroRadius);
        Assert.Equal(12, tuning.ChaseLeash);
        Assert.Equal(1, tuning.AttackRange);
        Assert.Equal(10, tuning.MonsterAttackDamage);
        Assert.Equal(1000, tuning.MonsterAttackCooldownMs);
        // De-aggro is derived ×1.5 of the acquire radius (6 → 9), always strictly beyond acquire.
        Assert.Equal(9, tuning.MonsterDeaggroRadius);
        Assert.True(tuning.MonsterDeaggroRadius > tuning.AggroRadius);
        // Attack cooldown ticks: 1000 ms @ 20 Hz = 20 ticks. Aggro scan ~0.5 s @ 20 Hz = 10 ticks.
        Assert.Equal(20u, tuning.MonsterAttackCooldownTicks);
        Assert.Equal(10u, tuning.MonsterAggroScanIntervalTicks);
    }

    [Fact]
    public void MonsterCombatKeysAreKnownButNotCombat()
    {
        foreach (var key in new[]
                 {
                     ServerTuningRegistry.MonsterAggroRadiusKey,
                     ServerTuningRegistry.MonsterChaseLeashKey,
                     ServerTuningRegistry.MonsterAttackRangeKey,
                     ServerTuningRegistry.MonsterAttackDamageKey,
                     ServerTuningRegistry.MonsterAttackCooldownMsKey,
                 })
        {
            Assert.True(ServerTuningRegistry.IsKnownKey(key));
            Assert.False(ServerTuningRegistry.IsCombatKey(key)); // server-only AI knob: no client re-broadcast.
    ...
    [Fact]
    public void MonsterCombatKeysApplyAndClamp()
    {
        var tuning = new ServerTuning(Options());

        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterAggroRadiusKey, 8d, out var a));
        Assert.Equal(8d, a);
        Assert.Equal(8, tuning.AggroRadius);
        Assert.Equal(12, tuning.MonsterDeaggroRadius); // derived ceil(8 * 1.5).

        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterAggroRadiusKey, 0d, out _));
        Assert.True(tuning.AggroRadius >= 1);
        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterAggroRadiusKey, 9999d, out _));
        Assert.True(tuning.AggroRadius <= 64);

        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterChaseLeashKey, 0d, out _));
        Assert.True(tuning.ChaseLeash >= 1);
        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterChaseLeashKey, 9999d, out _));
        Assert.True(tuning.ChaseLeash <= 128);

        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterAttackRangeKey, 0d, out _));
        Assert.True(tuning.AttackRange >= 1);
        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterAttackRangeKey, 99d, out _));
        Assert.True(tuning.AttackRange <= 4);

        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterAttackDamageKey, -5d, out _));
        Assert.True(tuning.MonsterAttackDamage >= 0);
        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterAttackDamageKey, 1_000_000d, out _));
        Assert.True(tuning.MonsterAttackDamage <= 10000);

        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterAttackCooldownMsKey, 0d, out _));
        Assert.True(tuning.MonsterAttackCooldownMs >= 100);
        Assert.True(ServerTuningRegistry.TryApply(tuning, ServerTuningRegistry.MonsterAttackCooldownMsKey, 1_000_000d, out _));
        Assert.True(tuning.MonsterAttackCooldownMs <= 10000);
    }
