...
public sealed class LocalPlayerPredictorTests
{
    ...
    [Fact]
    public void SwingSlow_UnderLatency_ServerAndPredictorSlowSameStepsToSameTicks()
    {
        // SWING-SLOW — THE REQUIRED PARITY PROOF (the latency-injecting test). A swing no longer FREEZES movement;
        // it SLOWS it by combat.swingMoveFactor for the swing window. This test drives the REAL server WorldEntity
        // and the predictor on the SAME 50 ms tick grid with a NON-ZERO factor (a genuine slow, not the old block),
        // injecting latency between when the client authors the swing (tick T) and when the server receives it
        // (T + latency). Both sides anchor the slow window on the SAME authored tick T (the server clamps it) and
        // slow with the SAME factor, so they must accept the IDENTICAL set of step ticks — through and after the
        // window — and stay tile/seq in lockstep every tick. A divergence in the anchor OR the factor OR the slowed-
        // cooldown formula makes the accepted-tick sets differ → this fails (it is exactly the swing-then-move
        // rubberband, now in its slow form).
        const int tickRate = 20;                 // 50 ms/tick
        const double tickMs = 1000d / tickRate;  // 50 ms
        const uint stepCooldownTicks = 3;        // 150 ms base cadence
        var stepCadenceMs = MovementCadence.EffectiveStepCadenceMs(150, tickRate);

        // A genuine slow (NOT 0 = block, NOT 1 = no slow): 40% speed => cooldown ceil(3 / 0.4) = ceil(7.5) = 8 ticks
        // inside the window. Pin the shared formula so a break in either side's math fails here, not just live.
        const double factor = 0.4d;
        var slowedCooldown = CombatTuning.SlowedStepCooldownTicks(stepCooldownTicks, factor);
        Assert.Equal(8u, slowedCooldown);
        Assert.True(slowedCooldown > stepCooldownTicks);   // it is genuinely slower
        Assert.NotEqual(CombatTuning.SwingBlockCooldownTicks, slowedCooldown); // and NOT a block

        // The slow WINDOW duration (the old root window): combat.rootMs default -> ticks. Both sides use this.
        var slowDurationTicks = CombatTuning.RootTicks(tickRate);
        Assert.Equal(slowDurationTicks, CombatTuning.RootTicksFromTickMs(tickMs)); // server/predictor agree
        ...
        const uint authorTick = 6;
        const uint latency = 2;                 // server receives the swing `latency` ticks after it is authored
        Assert.True(latency <= futureLead);     // authored tick is within the clamp window (used verbatim)
        ...
        // Hold E the whole time so every eligible tick is a MOVE attempt — the slow is the only thing that can
        // change the step cadence, isolating the swing-slow machinery.
        const Direction8 held = Direction8.E;
        predictor.SetIntent(true, held, TimeSpan.Zero);

        var serverStepTicks = new List<uint>();
        var predictorStepTicks = new List<uint>();

        for (uint tick = 0; tick <= 60; tick++)
        {
            // Predictor: opens its swing-slow window at the authored tick the INSTANT it swings (the same authoredTick
            // it stamps on the wire), with the replicated factor.
            if (tick == authorTick)
            {
                predictor.ApplyAttackMovementSlowAt(authorTick, slowDurationTicks, factor);
            }

            // Server: the attack arrives `latency` ticks later; it opens the slow window on the message's AUTHORED
            // tick (clamped to its window), NOT its receive tick, with the SAME factor.
            if (tick == authorTick + latency)
            {
                entity.ApplyAttackMovementSlowAuthored(authorTick, tick, slowDurationTicks, factor, pastWindow, futureLead);
            }

            var predictedSeqBefore = predictor.PredictedStepSeq;
            predictor.Tick(TimeSpan.FromMilliseconds(tick * tickMs));
            if (predictor.PredictedStepSeq > predictedSeqBefore)
            {
                predictorStepTicks.Add(tick);
            }

            if (entity.TryStep(held, tick, stepCooldownTicks, grid))
            {
                serverStepTicks.Add(tick);
            }

            // Tile / facing / accepted-step-count lockstep EVERY tick, through and after the slow window.
            Assert.Equal(entity.Tile, predictor.PredictedTile);
            Assert.Equal(entity.Facing, predictor.Facing);
            Assert.Equal(entity.StepSequence, predictor.PredictedStepSeq);
        }

        // The discriminating assertion: BOTH sides accepted steps on the IDENTICAL ticks — the slow stretched the
        // same steps to the same ticks on both. (On a divergent anchor/factor the late-window server would stretch a
        // DIFFERENT step set than the predictor and these would differ — the rubberband.)
        Assert.Equal(serverStepTicks, predictorStepTicks);

        // And the slow genuinely happened: at least one inter-step gap inside the window equals the slowed cooldown
        // (8 ticks), proving movement was slowed — not frozen (a block would show NO step inside the window) and not
        // unchanged (base 3 throughout). The window is [authorTick, authorTick + slowDurationTicks).
        var sawSlowedGap = false;
        for (var i = 1; i < serverStepTicks.Count; i++)
        {
            var prev = serverStepTicks[i - 1];
            var gap = serverStepTicks[i] - prev;
            if (prev >= authorTick && prev < authorTick + slowDurationTicks && gap == slowedCooldown)
            {
                sawSlowedGap = true;
            }
        }

        Assert.True(sawSlowedGap, "expected at least one step inside the swing window to be delayed by the slowed cooldown");
    }

    [Fact]
    public void SwingSlow_CommitPathPacedUnderLatency_ServerRetroSlowsFromAuthoredTick_NotPacedScheduledTick()
    {
        // SWING-SLOW COMMIT-PATH PARITY — the gap the held-intent SwingSlow_UnderLatency_... test is blind to (it
        // drives TryStep, NOT the live UoClientDriven TryCommitStepAuthored path). On the commit path the server's
        // retroactive swing-slow at window-open keyed on _lastStepTick = the PACED scheduledTick the commit is applied
        // at — but the predictor mirrors it from its pre-pacing GATE tick (_lastLiveStepTick). They coincide only when
        // pacing is a no-op; a recovered/bundled/REORDERED commit the server paces up to (_lastAuthoredCommitTick +
        // cooldown) makes scheduledTick > the authored gate tick, so the retroactive slowedNext diverged →
        // server↔predictor next-eligible mismatch (the slow-form rubberband). The fix anchors the retro-slow on the
        // AUTHORED tick the client banked — byte-identical on both sides — so both re-slow the in-window step to the
        // SAME next-eligible tick.
        //
        // This is a focused white-box reproduction. It drives the REAL server commit path into the exact paced state
        // (a reordered prior commit forces the in-window step's scheduledTick PAST its authored gate tick), opens the
        // swing window, and then asserts the next accepted commit tick. The PREDICTOR, driven to step at the SAME
        // authored gate tick and opening the SAME window, re-slows to `authoredGate + slowedCooldown`. The server MUST
        // resume at the same tick. Pre-fix it resumed at `pacedScheduled + slowedCooldown` (LATER) — this asserts the
        // post-fix value, so it FAILS pre-fix and passes after the fix.
        const int tickRate = 20;                 // 50 ms/tick
        const double tickMs = 1000d / tickRate;  // 50 ms
        const uint cd = 3;                        // 150 ms base cadence (3 ticks)
        var stepCadenceMs = MovementCadence.EffectiveStepCadenceMs(150, tickRate);

        const double factor = 0.4d;              // a genuine slow (not 0 = block, not 1 = no slow)
        var slowed = CombatTuning.SlowedStepCooldownTicks(cd, factor);
        Assert.Equal(8u, slowed);                // ceil(3 / 0.4) = 8
        var slowDur = CombatTuning.RootTicks(tickRate);     // window length
        Assert.Equal(slowDur, CombatTuning.RootTicksFromTickMs(tickMs)); // server/predictor agree on the window length
        ...
        // -------------------------------------------------------------------------------------------------------
        // SERVER: drive the real commit path into the PACED state, then open the swing window.
        //
        // Commit C1 authored @0 (accepted, scheduled 0). Commit C2 authored @1 — a REORDER/too-close authored tick
        // (1 < 0 + cd = 3) — so the server PACES it to scheduled = 3. Now _lastAuthoredCommitTick = 3, and the LAST
        // accepted step's AUTHORED tick is 1 (C2's clampedAuthored) while its applied/scheduledTick is 3.
        //   * pre-fix retro-slow anchors on _lastStepTick = 3  -> slowedNext = 3 + 8 = 11
        //   * post-fix retro-slow anchors on the authored tick = 1 -> slowedNext = 1 + 8 = 9
        // The window must cover both anchors so the retro-slow fires: open it at authored 1 with the default duration
        // (>= 3 ticks), so the window is [1, 1+slowDur) which contains 1 (post-fix anchor) AND 3 (pre-fix anchor).
        // -------------------------------------------------------------------------------------------------------
        var entity = new WorldEntity(2, 2, EntityKind.Player, new TileCoord(10, 10), Direction8.E,
            "Server", System.Guid.NewGuid(), ownerSession: null, isDurable: true);

        Assert.True(entity.TryCommitStepAuthored(Direction8.E, 0, 0, cd, pastWindow, futureLead, grid, out _)); // C1 @0
        Assert.True(entity.TryCommitStepAuthored(Direction8.E, 1, 1, cd, pastWindow, futureLead, grid, out var r2)); // C2 reorder
        Assert.Equal("committed", r2.Reason);
        Assert.Equal(2u, entity.StepSequence);
        // C2 was paced to scheduled 3 (authored 1 < prior 0 + cd 3). The window must contain BOTH anchors (1 and 3).
        Assert.True(slowDur >= 3, "window must cover the paced scheduled tick (3) so the bug would also fire pre-fix");

        // Swing authored @1, server receives at tick 1 (latency folded out; the clamp is a no-op since 1 is within the
        // window). This opens [1, 1+slowDur) and triggers the retroactive re-slow of the last accepted step, setting
        // _nextEligibleTick = retroAnchor + slowed.
        entity.ApplyAttackMovementSlowAuthored(authoredTick: 1, serverTick: 1, slowDur, factor, pastWindow, futureLead);

        // The player now RELEASES — movement returns to the held-intent path (WorldEntity.TryStep), which DOES gate on
        // _nextEligibleTick (the authored-commit path does not; that is why the mis-slowed _nextEligibleTick only
        // surfaces on the release/held-intent boundary — exactly the live transition off the commit stream). The first
        // tick TryStep accepts is the server's resume tick = its _nextEligibleTick. Sweep to find it.
        uint serverResumeTick = 0;
        var found = false;
        for (uint t = 2; t <= 40 && !found; t++)
        {
            if (entity.TryStep(Direction8.E, t, cd, grid))
            {
                serverResumeTick = t;
                found = true;
            }
        ...
        Assert.True(found, "the server must eventually accept a held-intent step after the swing window");

        // -------------------------------------------------------------------------------------------------------
        // PREDICTOR PARITY: the predictor's mirror (ApplyAttackMovementSlowAt) re-slows the last LIVE step from its
        // GATE tick (_lastLiveStepTick) — the SAME authored tick (1) the server now anchors on post-fix — so the
        // predictor's next-eligible is 1 + slowed = 9. The server MUST resume on that SAME tick.
        //   * POST-FIX: server anchors on the authored tick (1) -> _nextEligibleTick = 1 + 8 = 9 == predictor.  PASS
        //   * PRE-FIX:  server anchored on the PACED scheduled tick (3) -> _nextEligibleTick = 3 + 8 = 11 != 9.  FAIL
        // -------------------------------------------------------------------------------------------------------
        const uint authoredGateOfLastStep = 1u;          // the predictor's gate tick of its last in-window step
        var expectedResume = authoredGateOfLastStep + slowed;   // 1 + 8 = 9

        Assert.Equal(expectedResume, serverResumeTick);

        // Discriminator guard: the BUGGY paced anchor (scheduled 3) would have resumed at 3 + 8 = 11, NOT 9.
        Assert.NotEqual(3u + slowed, serverResumeTick);
    }

    [Fact]
    public void SwingSlow_CommitPathPaced_HeldIntentResumesInLockstepWithPredictor()
    {
        // Companion to the white-box test: drives the ACTUAL predictor mirror so the parity is proven against the
        // real LocalPlayerPredictor (not a hand-computed value). The predictor takes a step at its gate `anchor`,
        // opens the swing window anchored there, and we read its re-slowed next-eligible (its first post-window step).
        // The server is driven into the PACED state (a duplicate/reorder commit forces the last accepted commit's
        // scheduledTick PAST its authored tick `anchor`), the SAME window is opened, and it is released onto the
        // held-intent TryStep path (which gates on _nextEligibleTick). Both must resume on the SAME tick — the
        // end-to-end commit-path proof. Pre-fix the server re-slowed from the paced scheduledTick (later), failing it.
        const int tickRate = 20;
        const double tickMs = 1000d / tickRate;
        const uint cd = 3;
        var stepCadenceMs = MovementCadence.EffectiveStepCadenceMs(150, tickRate);
        const double factor = 0.4d;
        var slowed = CombatTuning.SlowedStepCooldownTicks(cd, factor);
        var slowDur = CombatTuning.RootTicks(tickRate);
        const uint pastWindow = 64;
        const uint futureLead = 4;
        var grid = new TileGrid(64, 64, []);

        // --- PREDICTOR: step once on its own gate grid, capture the gate tick of that last step (the `anchor`), open
        // the swing window anchored there, and read the tick its gate next re-opens (its re-slowed next-eligible). The
        // ABSOLUTE anchor value doesn't matter — only that the server is driven to anchor its retro-slow on the SAME
        // authored gate tick, and both use the SAME slowed cooldown, so they resume on the same tick. ---
        var predictor = new LocalPlayerPredictor(new TileCoord(10, 10), Direction8.E, stepCadenceMs,
            t => grid.IsWalkable(t), tickMs);
        predictor.SetClientDriven(true);
        predictor.SetIntent(true, Direction8.E, TimeSpan.Zero);

        // Step the predictor once on its own grid; capture its last gate tick, open the window anchored there.
        Span<Direction8> dirBuf = stackalloc Direction8[2];
        Span<long> tickBuf = stackalloc long[2];
        long lastGate = -1;
        for (uint tick = 0; tick <= 2; tick++)
        {
            predictor.Tick(TimeSpan.FromMilliseconds(tick * tickMs), dirBuf, tickBuf, out var n);
            for (var i = 0; i < n; i++)
            {
                lastGate = tickBuf[i];
            }
        ...
        Assert.True(lastGate >= 0, "predictor took at least one step");
        var anchor = (uint)lastGate;
        predictor.ApplyAttackMovementSlowAt(anchor, slowDur, factor);

        // Find the predictor's next step tick (its re-slowed next-eligible). Keep holding E and poll forward.
        uint predictorResume = 0;
        var predFound = false;
        for (uint tick = anchor + 1; tick <= anchor + 40 && !predFound; tick++)
        {
            predictor.Tick(TimeSpan.FromMilliseconds(tick * tickMs), dirBuf, tickBuf, out var n);
            if (n > 0)
            {
                predictorResume = (uint)tickBuf[0];
                predFound = true;
            }
        }

        Assert.True(predFound, "predictor resumes after the window");
        // The predictor re-slowed from its gate anchor: next-eligible == anchor + slowed.
        Assert.Equal(anchor + slowed, predictorResume);

        // --- SERVER: paced reorder (authored anchor-1 then anchor) so the last in-window commit is paced past its
        // authored tick, open the SAME window at the authored anchor, release onto TryStep, find resume. ---
        var entity = new WorldEntity(2, 2, EntityKind.Player, new TileCoord(10, 10), Direction8.E,
            "Server", System.Guid.NewGuid(), ownerSession: null, isDurable: true);

        // C1 authored @0 (accepted, schedule anchor 0). Then a second commit authored at `anchor` that is too-close to
        // the prior accepted authored tick (a reorder / duplicate-authored recovery): the pace gate (>= prior + cd)
        // forces its scheduledTick PAST `anchor`, so the LAST accepted step's authored tick stays `anchor` while its
        // applied scheduledTick is later. That is the exact paced state the retro-slow must anchor on the authored tick.
        Assert.True(entity.TryCommitStepAuthored(Direction8.E, 0, 0, cd, pastWindow, futureLead, grid, out _));
        entity.TryCommitStepAuthored(Direction8.E, anchor, anchor + 2, cd, pastWindow, futureLead, grid, out _);

        entity.ApplyAttackMovementSlowAuthored(anchor, anchor + 1, slowDur, factor, pastWindow, futureLead);

        uint serverResume = 0;
        var srvFound = false;
        for (uint t = anchor; t <= anchor + 40 && !srvFound; t++)
        {
            if (entity.TryStep(Direction8.E, t, cd, grid))
            {
                serverResume = t;
        ...
        Assert.True(srvFound, "server resumes after the window");

        // THE PARITY: the server resumes on the SAME tick the predictor does — both re-slowed from the AUTHORED gate
        // anchor. Pre-fix the server re-slowed from the paced scheduled tick (later), breaking this equality.
        Assert.Equal(predictorResume, serverResume);
    }
