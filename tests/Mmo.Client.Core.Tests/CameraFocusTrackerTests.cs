...
// S95 — unit tests for the pure camera-focus math (frame-rate-independent smoothing + teleport snap), the
// Godot-free seam behind UpdateCamera. No Godot types: plain doubles, deterministic deltas. The former
// confirmed-tile follow-blend leg was removed (tile-era remnant); the camera tracks the continuous render
// position directly.
public sealed class CameraFocusTrackerTests
{
    ...
    public void FirstFrameSeedsToTargetInsteadOfGlidingFromOrigin()
    {
        ...
        // smoothing 10/s — but on the very first frame it must SNAP, not glide from (0,0).
        var (x, y) = tracker.Advance(50, 50, smoothingPerSecond: 10, deltaSeconds: 0.016, teleportSnapDistance: SnapTiles);
    ...
    [Fact]
    public void SmoothingOffFollowsTargetExactly()
    {
        var tracker = new CameraFocusTracker();
        // smoothing 0 => hard-follow the (continuous) render target exactly.
        var (x, y) = tracker.Advance(12.5, 22.5, smoothingPerSecond: 0, deltaSeconds: 0.016, teleportSnapDistance: SnapTiles);
    ...
    public void SmoothingConvergesAtTheSameWallClockSpeedRegardlessOfFrameRate()
    {
        ...
        coarse.Advance(0, 0, 0, 0, noSnap); // seed at origin
        var fine = new CameraFocusTracker();
        fine.Advance(0, 0, 0, 0, noSnap);

        // 10 steps of 0.1s
        for (int i = 0; i < 10; i++)
        {
            coarse.Advance(10, 10, rate, 0.1, noSnap);
        }
        // 100 steps of 0.01s — same total time.
        for (int i = 0; i < 100; i++)
        {
            fine.Advance(10, 10, rate, 0.01, noSnap);
        }
    ...
    {
        var tracker = new CameraFocusTracker();
        tracker.Advance(0, 0, 0, 0, SnapTiles); // seed at origin
        var (x, _) = tracker.Advance(2, 0, 10, 0.016, SnapTiles);
    ...
    public void TeleportBeyondThresholdSnapsInstantlyEvenWithSmoothingOn()
    {
        ...
        var (x, y) = tracker.Advance(100, 0, 10, 0.016, SnapTiles);
    ...
    public void SmallMoveWithinThresholdSmoothsRatherThanSnaps()
    {
        ...
        var (x, _) = tracker.Advance(3, 0, 10, 0.016, SnapTiles);
