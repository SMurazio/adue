...
| 5 | Remote interpolation/extrapolation; retire hop/TileInterpolator | **IN PROGRESS** | plan: `docs/migration/phase-5-plan.md`. DECISION: position-sample INTERPOLATION (no wire change — smooths both continuous players + tile-stepped monsters); velocity-extrapolation DEFERRED (RemoteContinuousEntity kept hybrid-ready). Last phase before the drivable build. |
