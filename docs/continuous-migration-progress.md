...
| 8 | Monster AI: continuous nav + avoidance; Euclidean leash/aggro; HOPS preserved | **PLANNING** | **DECISION: slimes keep HOPPING** (discrete leaps + pause). Phase 8 makes NAVIGATION continuous (Euclidean leash/aggro/de-aggro, obstacle avoidance, sub-tile hop targets) but the slime still hops (Velocity~0, sparse updates → the jump). Future glide types deferred. |
