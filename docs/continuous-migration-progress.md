...
| 4 | Client prediction + reconcile (port `ContinuousPredictor`) | **IN PROGRESS** | plan: `docs/migration/phase-4-plan.md`. 3 determinism gaps resolved (replicate radius on ServerHello, shared `NeighborhoodWallsForMove`, derive speed from cadence); lift `MaxInputDtSeconds` shared. Ports the timing-faithful harness (the UO5/NET2/NET3 guard) + Followup A. First live-playable. |
