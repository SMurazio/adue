...
2. **(N3 review LOWs, deferred)** NodeFieldView never observes the N2 disconnect-clear (inert today —
   Disconnect only fires at scene teardown — but whoever adds live reconnect must reset _zoneBuilt and
   free the view; noted trap). DebugControlChannel.cs:358 still emits the constant-false Depleted per
   entity on client_entities (dead diagnostic noise; sweep when next touched). ModelVisual.CreateRock/
   CreateTree unreachable-but-documented (dies with the art pass).
3. **Validation-order reply delta (LOW, accepted).** Harvest answers "depleted" before "too_far" (old
