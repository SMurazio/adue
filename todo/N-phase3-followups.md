...
## A — DONE (Phase 4)
Added `tests/Mmo.Server.Tests/RawDirectionNormalizeIntegrationTests.cs` — a socket integration test driving RAW
non-unit `MoveIntent`s over the real receive path: a raw `(10,0)` integrates within a robust band of the cardinal
`(1,0)` (no 10× magnitude boost / no throttle), and a raw `(1,1)` advances east SLOWER per-axis than cardinal (the √2
boost is normalized away). Guards `GameServer.HandleMoveIntent`'s `rawDir.Normalized()` against a regression that
would reopen the speed exploit.

## A (original note) — server raw-direction-normalize guard test (RECOMMENDED, do in/around Phase 4)
