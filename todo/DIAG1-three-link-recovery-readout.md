...
## Why
The local prediction strands ahead of the confirm under loss and **never recovers** (a permanent desync). A
permanent desync means ONE of three links is broken; we need to SEE which before fixing:
...
## Loss points to measure (per the new degradation brackets — see `docs/movement-loss-degradation-tiers.md`)
Do NOT fix on a single 3%. **Sweep the brackets and find the LOWEST loss where the permanent strand first
appears** — that is the cleanest, least-noisy place to diagnose:
- **≤1%** — must be **seamless** (lead drains, NO strand). If it strands even here, the bug is blatant and this
  is the best repro to read.
- **1–3%** — slight transient jitter is acceptable, but the lead must still **drain** (no permanent strand).
- (reference: **>5%** is failure-state = NET4 territory, not what DIAG1 is isolating.)
The human reads the live numbers at **1% and 3%** (add 0.5% / 5% if useful) and reports, per loss level,
whether the lead drains and which link is stuck.
...
## Acceptance
Under 100ms, **across ~1% and 3% loss**, the human can read `pred / conf / lead / srvSeq / recvCommits /
rejects` live and we can state (a) the **lowest loss where the lead stops draining** and (b) **which of the 3
links is stuck** there. Measurement only — no movement behavior change. Gates green.
